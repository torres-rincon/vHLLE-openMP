...
gsl_interp_accel* threadLocalAccel(const gsl_spline* spline) {
 thread_local unordered_map<const gsl_spline*, unique_ptr<gsl_interp_accel, GslInterpAccelDeleter>> accels;
 auto& accel = accels[spline];
 if (!accel) {
...
#if defined TABLE
 return gsl_spline_eval(spline_p, e, threadLocalAccel(spline_p));
#elif defined SIMPLE
 return e / 3.;
#endif
}
...
#if defined TABLE
 return (gsl_spline_eval(spline_p, e*1.1, threadLocalAccel(spline_p)) -
         gsl_spline_eval(spline_p, e, threadLocalAccel(spline_p))) /
        (0.1 * e);
#elif defined SIMPLE
 return 1. / 3.;
#endif
...
#if defined TABLE
 return gsl_spline_eval(spline_T, e, threadLocalAccel(spline_T));
#elif defined SIMPLE
 const double cnst =
...
#if defined TABLE
 return gsl_spline_eval(spline_mu, e, threadLocalAccel(spline_mu));
#elif defined SIMPLE
 return 0.;
#endif
}
