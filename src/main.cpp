...
bool freezeoutOnly {true}, freezeoutExtend {false}, vorticityOn {false};
...
void readParameters(char *parFile) {
    ...
    map<string, function<void(const string&)>> handlers = {
        ...
        {"freezeoutOnly", [](const string&) { freezeoutOnly = true; }},
...
int main(int argc, char **argv) {
 ...
 printParameters();
 const char *timingEnv = std::getenv("VHLLE_TIMING");
 const bool profileTiming = timingEnv != nullptr && timingEnv[0] != '\0' && strcmp(timingEnv, "0") != 0;
 double timingVtk = 0.0, timingPerformStep = 0.0, timingOutputSurface = 0.0,
        timingOutputGnuplot = 0.0, timingOutputCorona = 0.0;
 int timingSteps = 0;
 auto runTimed = [&](const char *label, double &accumulator, auto &&action) {
  if (profileTiming) {
   cout << "Timing checkpoint: start " << label << " step=" << timingSteps + 1 << endl;
   auto timingStart = std::chrono::steady_clock::now();
   action();
   const double elapsed = elapsedSeconds(timingStart);
   accumulator += elapsed;
   cout << "Timing checkpoint: end " << label << " step=" << timingSteps + 1
        << " elapsed=" << elapsed << " total=" << accumulator << endl;
  } else {
   action();
  }
 };
 ...
 if(icModel!=10) {
   runTimed("outputCorona", timingOutputCorona, [&]() { f->outputCorona(tau0, freezeoutExtend); });
 ...
 do {
  ...
  if (!vtk_values.empty()) {
   runTimed("vtk", timingVtk, [&]() { vtk_out.write(*h,vtk_values); });
  }
  int nSubSteps = 1;
  ...
  if(nSubSteps>1) {
   runTimed("performStep", timingPerformStep, [&]() {
   ...
   runTimed("performStep", timingPerformStep, [&]() { h->performStep(); });
  }
  ...
  runTimed("outputSurface", timingOutputSurface, [&]() {
   if (icModel == 10) {
  ...
  if (!freezeoutOnly) {
   runTimed("outputGnuplot", timingOutputGnuplot, [&]() { f->outputGnuplot(ctime); });
  }
  timingSteps++;
 ...
 if (profileTiming) {
  cout << "Timing profile [sec]:"
       << " steps=" << timingSteps
       << " vtk=" << timingVtk
       << " outputCorona=" << timingOutputCorona
       << " performStep=" << timingPerformStep
       << " outputSurface=" << timingOutputSurface
       << " outputGnuplot=" << timingOutputGnuplot
       << endl;
 }
