...
+ main branch (RECOMMENDED) contains the most recent, feature-rich code
 ...
 -> on Ubuntu run:
 sudo apt install make g++ binutils gsl-bin libgsl-dev
 -> on Fedora run:
 sudo yum install make gcc-c++ binutils gsl gsl-devel
 ...
 The default build is serial and creates the executable:
 ./hlle_visc

 To build the OpenMP-parallel version, run:
 make OPENMP=1

 The number of OpenMP threads is selected when running the code, not when
 compiling it. For example, to run with 8 threads:
 OMP_NUM_THREADS=8 ./hlle_visc -params <parameter-file> [-system <system>] -ISinput <IS-file> -outputDir <output-directory>

 In general, replace 8 by the desired number of threads:
 OMP_NUM_THREADS=<N> ./hlle_visc -params <parameter-file> [-system <system>] -ISinput <IS-file> -outputDir <output-directory>

 On Linux, the Makefile uses GCC's standard OpenMP flag (-fopenmp) when
 OPENMP=1 is set. On macOS, install libomp (for example with Homebrew) and
 use the same build command:
 make OPENMP=1

 If GSL is installed in a non-standard location, specify it explicitly:
 make OPENMP=1 GSL_PREFIX=/path/to/gsl
